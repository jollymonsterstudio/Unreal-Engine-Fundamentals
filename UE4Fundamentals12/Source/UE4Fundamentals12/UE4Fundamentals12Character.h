...
#include "UE4Fundamentals12Character.generated.h"
...
UCLASS(config=Game)
class AUE4Fundamentals12Character : public ACharacter
{
	GENERATED_BODY()
...
private:
	...
	* @param CurrentLogLevel {@see ELogLevel} affects color of log
	* @param FString the message for display
	*/
	void Log(ELogLevel CurrentLogLevel, FString Message);
	/**
	* Log - prints a message to all the log outputs with a specific color
	* @param CurrentLogLevel {@see ELogLevel} affects color of log
	* @param FString the message for display
	...
	*/
	void Log(ELogLevel CurrentLogLevel, FString Message, ELogOutput LogOutput);
};
